...
// botComment returns a visible attribution footer with the invisible bot marker embedded.
// The marker starts with "<!-- oompa-bot" so botMarker detection still works.
func (a *Agent) botComment() string {
	marker := "<!-- oompa-bot -->"
	if a.cfg.Version != "" {
		marker = fmt.Sprintf("<!-- oompa-bot v:%s -->", shortSHA(a.cfg.Version))
	}
	return fmt.Sprintf("---\n*🤖 Generated by [oompa](https://github.com/qinqon/oompa). Use `/oompa` to talk with me.* %s", marker)
}
