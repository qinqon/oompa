...
func newCountingWebhook(t *testing.T) (reporter *Reporter, posts func() int) {
	t.Helper()
	...
	t.Cleanup(ts.Close)
	reporter = NewReporter(ts.URL, "", discardLogger())
...
func TestFormatSlackMessage_EmptyFindings(t *testing.T) {
	...
	body = formatSlackMessage([]Finding{}, "")
	if body != nil {
...
func TestSlackReporter_Disabled(t *testing.T) {
	...
	reporter.Report(context.Background(), []Finding{
...
func TestSlackReporter_FlushDedup_WithinBatch(t *testing.T) {
	...
	logger := discardLogger()
	reporter := NewReporter(ts.URL, "", logger)
	reporter.httpClient = ts.Client()
